...
float max = 100.0;
float range;
int float_per_thread;
int b /* number of bins */, 
t /* thread_count */;
omp_lock_t * lock_ary; /* lock array */
...
void fill_bin(int * bin, float * flt, int n_f)
{
    int my_rank = omp_get_thread_num();
    int start = my_rank * float_per_thread,
    end = (my_rank == t - 1) ? n_f : start + float_per_thread; 
    for(int i = start; i < end; ++i)
    {
        float target = flt[i + 1]; /* i + 1 since *flt is number of floats */
        int idx = find_idx(target, range, max);
        omp_set_lock(lock_ary + idx);
        ++bin[idx];
        omp_unset_lock(lock_ary + idx);
    }
}
...
int main(int argc, char ** argv)
{
    ...
    range = max / b;
    lock_ary = (omp_lock_t *)alloc(b * sizeof(omp_lock_t));
    for(int i = 0; i < b; ++i)
        omp_init_lock(lock_ary + i);
