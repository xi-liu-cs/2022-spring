...
if(!my_rank)
  recvbuf2 = malloc((n / x + comm_sz) * sizeof(int));
sendbuf2 = ret_buf;

MPI_Gather(sendbuf2, sendcounts[my_rank] / x, MPI_INT, recvbuf2, ret_i, MPI_INT, 0, MPI_COMM_WORLD);
MPI_Finalize();

if(!my_rank)
{
  int recv2_i = 0;
  while(recv2_i < n / x && i < n / x + comm_sz)
  {
    if(recvbuf2[recv2_i])
    {
      printf("i = %d, r = %d\n", i, recvbuf2[i]);
