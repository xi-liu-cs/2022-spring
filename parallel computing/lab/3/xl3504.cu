...
(
    ...
    float * bd,
    float * cd,
    int th_work, /* thread work */
    int blk_per_grid, /* block_per_grid */
    int th_per_blk, /* thread_per_block */
    int n
)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
    int i = id * th_work;
    if(id == blk_per_grid * th_per_blk - 1)
        th_work = n - i + 1;
	for(int work = 0; work < th_work; ++work)
	{
		if(i < n)
		{
			cd[i] += ad[i] * bd[i];
