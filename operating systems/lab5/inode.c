...
inode_block_walk(struct inode *ino, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if (filebno < N_DIRECT){
		*ppdiskbno = &(ino->i_direct[filebno]);
		...
		if (!ino->i_indirect){
			if (!alloc){return -ENOENT;}
			uint32_t blockno = alloc_block();
			if (blockno<0){return -ENOSPC;}
			ino->i_indirect = blockno;

			uint32_t *addressptr = diskaddr(ino->i_indirect);
			*ppdiskbno = &(addressptr[filebno-N_DIRECT]);
			return 0;
		}
	}
		...
		if (!ino->i_double){
			...
			uint32_t firstBlockno = alloc_block();	
			if (firstBlockno<0){return -ENOSPC;}
			ino->i_double = firstBlockno;

			uint32_t secondBlockno = alloc_block();	
			if (secondBlockno<0){return -ENOSPC;}

			// assign this second allocated block to firstblock/updated i_double[index]
			uint32_t *addressptr = diskaddr(ino->i_double);
			addressptr[filebno-N_DIRECT] = secondBlockno;
			*ppdiskbno = &(addressptr[filebno-N_DIRECT-N_INDIRECT]);
			return 0;
		}
	}
	return -EINVAL;
...
inode_get_block(struct inode *ino, uint32_t filebno, char **blk)
{
	...
	if (filebno > (N_DIRECT+N_INDIRECT+N_DOUBLE)){return -EINVAL;}
	int blockTest;
	uint32_t *pdiskbno;
	blockTest = inode_block_walk(ino, filebno, &pdiskbno, true);	
	if (blockTest!=0){return blockTest;}

	if (!*pdiskbno){	
		*pdiskbno = alloc_block();
		if (*pdiskbno == -ENOSPC){return ENOSPC;}
	}
...
inode_truncate_blocks(struct inode *ino, uint32_t newsize)
{
	...
	else if (new_nblocks < (N_DIRECT+N_INDIRECT+N_DOUBLE)){
		free_block(ino->i_indirect);
...
{
	// LAB: Your code here. ex5
	// panic("inode_unlink not implemented");

	struct inode *pdir, *pino;
	struct dirent *pdent;
	char *lastelem = malloc(sizeof(char) * NAME_MAX);
	int flag = walk_path(path, &pdir, &pino, &pdent, lastelem);
	if (flag<0){return -ENOENT;}

	// decrement link# or free the block if link#==0
	if (pino->i_nlink > 0){pino->i_nlink--;}
	else{inode_free(pdent->d_inum);}
	
	// zero out pdent
	pdent->d_inum = 0;
	memset(pdent->d_name, 0, NAME_MAX);
	return 0;
...
inode_link(const char *oldpath, const char *newpath)
{
	...
	struct inode* oldPdir, *oldPino, *newPdir, *newPino ;
	struct dirent* oldPdent, *newPdent;
	char oldLastelem[NAME_MAX];
	char newLastelem[NAME_MAX];

	//find ino and dent where old and new path holds 
	walk_path(oldpath, &oldPdir, &oldPino, &oldPdent, oldLastelem);
	walk_path(newpath, &newPdir, &newPino, &newPdent, newLastelem);
	if (newPdent){return -EEXIST;}
	oldPino->i_nlink ++;

	// allocate and copy data for newpath's dirent
	flag = dir_alloc_dirent(newPdir, &newPdent);
	if (flag<0){return flag;}
	newPdent->d_inum = oldPdent->d_inum;
	strcpy(newPdent->d_name, newLastelem);
	return 0;
